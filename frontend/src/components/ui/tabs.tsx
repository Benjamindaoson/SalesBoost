...
export function TabsRoot({ defaultValue, children, className }: { defaultValue: string, children: React.ReactNode, className?: string }) {
    const [value, setValue] = React.useState(defaultValue);
    return (
        <div className={cn("w-full", className)}>
            {React.Children.map(children, child => {
                if (React.isValidElement(child)) {
                    // Inject props into TabsList and TabsContent
                    // This is a simplified version of what Radix does
                         ...
                         return React.cloneElement(child as any, { 
                             children: React.Children.map(child.props.children, trigger => {
                                 if (React.isValidElement(trigger) && trigger.type === TabsTrigger) {
                                     return React.cloneElement(trigger as any, { activeValue: value, onValueChange: setValue })
                                 }
                                 return trigger;
                             })
                         })
