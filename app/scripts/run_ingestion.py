...
Usage:
python -m app.scripts.run_ingestion --dir "D:\\SalesBoost\\销冠能力复制数据库"
"""
import os
import argparse
import asyncio
...
class DataIngestionPipeline:
    ...
    async def ingest_vectors(self, chunks: List[Document]):
        ...
        for i in tqdm(range(0, len(chunks), batch_size), desc="Vectorizing"):
            ...
            # 使用 KnowledgeService 添加文档
            # 注意：KnowledgeService.add_document 实际上是单个添加，这里我们为了效率可能需要扩展它支持批量
            # 或者直接调用其内部 collection.add (如果是 Chroma)
            # 为了保持封装，我们循环调用 add_document (可能较慢) 或 模拟批量
...
async def main():
    ...
    parser.add_argument("--dir", type=str, default="D:\\SalesBoost\\销冠能力复制数据库", help="Data directory path")
