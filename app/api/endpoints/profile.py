...
from app.models.runtime_models import Session, EvaluationLog
...
async def get_strategy_profile(
    ...
    """Get the user's high-level strategy profile."""
    # Ensure profile exists or calculate it
...
async def get_skill_profile(
    ...
    """Get recent skill evaluation history."""
    query = (
...
async def get_skill_trajectory(
    ...
    """
    Get time-series data of user skills.
    Returns trajectories for key dimensions: Integrity, Relevance, Correctness, Logic, Compliance.
    """
    # Calculate cutoff date
    # For MVP, we just get last N records since we don't have real dates in simulation sometimes
    limit = days * 5 # Approx 5 turns per day? Or just limit records.
    
    query = (
        select(EvaluationLog)
        .join(Session)
        .where(Session.user_id == user_id)
        .order_by(EvaluationLog.created_at.asc()) # Oldest first for graph
        # .where(EvaluationLog.created_at >= cutoff) # If real dates used
    )
    result = await db.execute(query)
    logs = result.scalars().all()
...
async def get_strategy_gap(
    ...
    """
    Compare user strategy success rates against Top Performers.
    """
    # 1. Get User Stats
    deviation_stats = await analyzer.get_strategy_deviation_stats(db, user_id, limit=20)
    
    # 2. Define Benchmarks (Hardcoded for MVP, should be dynamic later)
    # Benchmark: >85% optimal rate is Top Performer
...
async def explain_lack_of_improvement(
    ...
    """
    Explain why the user is not seeing skill improvement.
    """
    ...
    if adoption_rate < 0.3:
        reasons.append("Low Adoption Rate: You rarely try the Coach's suggestions.")
    ...
    if effective_rate < 0.4 and adoption_rate >= 0.3:
        reasons.append("Ineffective Execution: You try the suggestions, but execution needs polish.")
        evidence.append(f"Effectiveness Rate: {effective_rate*100:.1f}% (Benchmark: >60%)")
        
    # 3. Strategy Deviation
    deviations = [d for d in deviation_stats if d["is_deviation"]]
    if deviations:
        top_dev = deviations[0]
        reasons.append(f"Strategy Mismatch: In '{top_dev['situation_type']}', you often choose suboptimal strategies.")
        evidence.append(
            f"Situation: {top_dev['situation_type']}. "
            f"You chose: '{top_dev['strategy_chosen']}'. "
            f"Top Performers use: '{top_dev['golden_strategy']}'."
        )

    if not reasons:
        reasons.append("Data Insufficient or Performance Good: No obvious blockers found yet.")
...
async def explain_curriculum_choice(
    ...
    """
    Explain the reasoning behind the current curriculum recommendation.
    """
    ...
    explanation = {
        ...
        "why_this_situation": f"Your optimal decision rate in '{focus.focus_situation}' is significantly below benchmark.",
        "why_this_strategy": f"Top performers achieve 85%+ success with '{focus.focus_strategy}' here.",
